...
export async function GET(request: NextRequest) {
  ...
  if (searchParams.has("crn")) {
    addQuery(query, "CRN", searchParams.get("crn") as string);
  }
  console.log(query);
  ...
  try {
    ...
    if (
      searchParams.has("requisites") &&
      searchParams.get("requisites") != null
    ) {
    ...
    // Insert splitting logic here:
    cursor.forEach((course) => {
      course._id = course._id.split(" ")[1];
    });
