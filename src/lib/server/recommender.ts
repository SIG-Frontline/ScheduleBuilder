...
export type ClassWild = {
	name?: string; 					// the name of the class
	type: ClassRecType.WILDCARD; 	// the type of this recommendation
	course: string; 				// the course code (CS 100)
	legacy?: boolean; 				// whether this is legacy (no longer available)
 ...
 *
 */
export async function getRecommendedClasses(degree: string, major: string, catalogYear: string, takenCourses: string[]) : Promise<ClassRecommendation[]> {
	...
	if (!curricula || curricula.error) {
		...
		return [] as ClassRecommendation[];
	}

	// Creates a tree of all courses required for a degree, minus legacy and prereq locked courses
	const curriculaTree = await buildTree(curricula.classes, takenCourses);

	// Filters out courses that the user has already taken
	const recommendations = filterTree(curriculaTree, takenCourses);
...
async function buildTree(node: ClassRecNode[], takenCourses: string[]) : Promise<ClassRecommendation[]> {
	let classes = [] as ClassRecommendation[];
	for (const classTree of node) classes = classes.concat(await parseTree(classTree, takenCourses));
	return classes;
}

async function parseTree(node: ClassRecNode, takenCourses: string[]) : Promise<ClassRecommendation[]> {
	// Required class
	if ('name' in node) {
		...
		if (!takenCourses.includes(node.course)) {
			...
			if (!await check_prereq(node.course, takenCourses)) return []; // Checks the prereqs for that course
		}
	...
	if (Array.isArray(node) && node[0] == '$COND') {
		...
		for (let i = 1; i < node.length - 1; i += 2) {
			...
			if (condition) return buildTree(node[i + 1] as ClassRecNode[], takenCourses);
		}

		// Return the else part
		return buildTree(node[node.length - 1] as ClassRecNode[], takenCourses);
	}
	...
	if (Array.isArray(node) && node[0]) {
		...
		for (let i = 4; i < node.length; i++) classes = classes.concat(await parseTree(node[i] as ClassRecNode[], takenCourses));
...
function filterTree(node: ClassRecommendation[], takenCourses: string[]) : ClassRecommendation[] {
	let classes = [] as ClassRecommendation[];
	for (const classTree of node) classes = classes.concat(parseFilterTree(classTree, takenCourses));
	return classes;
}

function parseFilterTree(node: ClassRecommendation, takenCourses: string[]) : ClassRecommendation[] {
	if (node.type == ClassRecType.CLASS) {
		...
		if (node.operator == '&') {
			for (let i = 0; i < node.classes.length; i++) classes = classes.concat(parseFilterTree(node.classes[i] as ClassRecommendation, takenCourses));
			...
			while (node.numClasses != oldClassCount || node.numCredits != oldCreditCount) {
				...
				for (let i = 0; i < node.classes.length; i++) {
					const classesSubset = parseFilterTree(node.classes[i] as ClassRecommendation, takenCourses);
