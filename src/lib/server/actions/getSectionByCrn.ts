 ...
 */
export async function getSectionByCrn(term: number, crn: Array<string>) {
  const baseURL = `http://0.0.0.0:${process.env.PORT}`;
  const crnList = crn.toString();
  console.log("attempting to display crn list:");
  console.log(crnList);
  const URL = `${baseURL}/api/courses?term=${term}&crnList=${crnList}`;
  const data = fetch(URL)
    ...
    .then((data) => {
      ...
      // TODO: Keep an eye on this change. was previously returning data.courses[0];
      console.log("data.courses returned from gbcrn is: ");
      console.log(data.courses);
      return data.courses;
    })
    .then((courses) => {
      // console.log("The courses value now is: ");
      // console.log(courses);
      courses.forEach((course) => {
        // console.log("The course value for each course now is: ");
        // console.log(course);
